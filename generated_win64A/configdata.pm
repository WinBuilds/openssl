...
our %config = (
  ...
  openssldir => "",
  options => " no-asan no-asm no-crypto-mdebug no-crypto-mdebug-backtrace no-devcryptoeng no-ec_nistp_64_gcc_128 no-egd no-external-tests no-fuzz-afl no-fuzz-libfuzzer no-heartbeats no-makedepend no-md2 no-msan no-rc5 no-sctp no-ssl-trace no-ssl3 no-ssl3-method no-tls13downgrade no-ubsan no-unit-test no-weak-ssl-ciphers no-zlib no-zlib-dynamic",
  perl_archname => "MSWin32-x64-multi-thread",
  perl_cmd => "C:\\Strawberry\\perl\\bin\\perl.exe",
  perl_version => "5.22.0",
  perlargv => [ "VC-WIN64A", "no-asm", "no-makedepend" ],
  perlenv => {
      "AR" => undef,
      "ARFLAGS" => undef,
      "AS" => undef,
